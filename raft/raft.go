...
func (rn *RaftNode) sendCommit() {
	// FIX: Catch and ignore panics if the simulator closes the commit channel
	defer func() { recover() }()
...
func (rn *RaftNode) Submit(command interface{}) (bool, interface{}, error) {
	...
	if rn.state == Leader {
		...
		case AddServers:
			...
			for i := 0; i < len(serverIds); i++ {
				newId := uint64(serverIds[i])
				rn.peerList.Add(newId)
				rn.server.peerList.Add(newId)
				rn.nextIndex[newId] = uint64(len(rn.log)) + 1
				rn.matchIndex[newId] = 0

				// Establish actual TCP Connections for added servers
				addr := &net.TCPAddr{IP: net.ParseIP("127.0.0.1"), Port: 2000 + int(newId)}
				rn.server.ConnectToPeer(newId, addr)
			}
			rn.persistToStorage()
		...
		case RemoveServers:
			...
			for i := 0; i < len(serverIds); i++ {
				targetId := uint64(serverIds[i])
				if rn.id != targetId {
					rn.peerList.Remove(targetId)
					rn.server.peerList.Remove(targetId)

					// Tear down TCP connections
					rn.server.DisconnectPeer(targetId)
				}
			}
			rn.persistToStorage()
			rn.debug("log=%v", rn.log)
